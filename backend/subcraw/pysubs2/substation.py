...
def color_to_ass_rgba(c):
    return "&H%08X" % ((c.a << 24) | (c.b) | (c.g << 8) | c.r << 16)
        ...
        def field_to_string(f, v, line):
            if f in {"start", "end"}:
                ...
                if all(ord(c) < 128 for c in v):
                    ...
                    raise TypeError(
                        "Encountered binary string with non-ASCII codepoint in SubStation field {!r} for line {!r} - please use unicode string instead of str".format(
                            f, line))
