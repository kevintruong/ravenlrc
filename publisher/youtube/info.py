...
channelinfodir = ContentDir.CHANNELINFO_DIR
...
class ChannelInfoManger:
    ...
    def get_channel_info_config(channelname):
        ContentDir.get_file_path(ContentDir.CHANNELINFO_DIR, '{}.json'.format(channelname))
...
class YoutubeMVInfo:
    def __init__(self, channelname: str, mv_info: SongInfo):
        self.channel = channelname
        # self.mv_info = mv_info
        self.songinfo = mv_info
    ...
    def create_hashtags(self):
        singer = create_hashtag(self.songinfo.singer)
        song = create_hashtag(self.songinfo.title)
        return [singer, song]

    def create_yt_tags(self):
        hashtags = generate_singer_song_hashtags(self.songinfo.singer, self.songinfo.title)
        final_hashtags = hashtags + self.channelinfo.footer.tags
        lenth = 0
        ...
        while lenth > 400:
            ...
            for each in final_hashtags:
                lenth = lenth + len(each)
        print(lenth)
        songinfo_hashtags = ",".join(final_hashtags)
        return songinfo_hashtags
    ...
    def description_formatter(self):
        ...
        description = description + (
            '-------------------------------------------------------------------------------------\n')
        if self.songinfo.lyrictext:
            description = description + self.songinfo.lyrictext
        description = description + (
            '-------------------------------------------------------------------------------------\n')
...
class YtMvConfigSnippet:
    ...
    @classmethod
    def create_snippet_from_info(cls,
                                 info: YoutubeMVInfo):
        return YtMvConfigSnippet(info.title,
                                 info.description,
                                 info.tags,
                                 info.hashtags)
        pass
